    ...
    try:
        ...
        if job.lora_model:
            ...
            if not Path(lora_path).exists() and job.lora_model.air:
                ...
                lora_path = download_lora(job.lora_model.air, lora_path, settings.CIVITAI_API_KEY)
        ...
        logger.debug(
            f"Input params: {gen_params['width']}x{gen_params['height']}, steps={gen_params['steps']}, cfg={gen_params['guidance_scale']}, seed={gen_params.get('seed')}, scheduler={gen_params.get('scheduler')}"
        )
        ...
        for idx in range(num_images):
            ...
            def gen_progress(*args, **kwargs):
                ...
                if not callback_called[0]:
                    logger.info(
                        f"  Callback CALLED! args={len(args)}, kwargs={list(kwargs.keys())}"
                    )
            ...
            if job.identifier:
                ...
                filename = (
                    f"{job_id:05d}.{img_no:02d}-p{prompt_id:03d}-m{model_id:03d}-l{lora_id:03d}.jpg"
                )
        ...
        generation_metadata = {
            ...
            "prompt": {
                ...
                "final": first_pipeline_meta.get(
                    "prompt", gen_params["prompt"]
                ),  # Use actual prompt with LoRA triggers
                "negative": first_pipeline_meta.get(
            ...
            "parameters": {
                ...
                "guidance_scale": first_pipeline_meta.get(
                    "guidance_scale", gen_params["guidance_scale"]
                ),  # Use ACTUAL guidance_scale
