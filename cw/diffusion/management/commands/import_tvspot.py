...
from cw.diffusion.models import TvSpot, TvSpotScriptRow, TvSpotVersion
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--dry-run", action="store_true", help="Validate only, do not create records"
        )
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            ...
            self.stdout.write(
                self.style.SUCCESS(f"  ✓ Created {len(data['script_rows'])} script rows")
            )
