...
from cw.diffusion.models import DiffusionModel, LoraModel, Prompt, DiffusionJob
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if dry_run:
            ...
            if lora_settings:
                self.stdout.write(f"  LoRA: {lora_settings.get('air', lora_settings.get('path', 'unknown'))}")
        ...
        if lora_settings:
            lora_model = self._get_or_create_lora(
                lora_settings,
                diffusion_model.base_architecture
            )
