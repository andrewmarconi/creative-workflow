            ...
            for idx, prompt_data in enumerate(prompts, 1):
                ...
                if options['skip_duplicates']:
                    if Prompt.objects.filter(source_prompt=source).exists():
                        skipped_count += 1
                        self.stdout.write(
                            self.style.WARNING(f"  [{idx}/{len(prompts)}] Skipped (duplicate): {source[:60]}...")
                        )
                        continue
                ...
                Prompt.objects.create(
                    ...
                    enhancement_method=prompt_data.get('enhancement_method', 'none') if not prompt_data.get('enhanced_prompt') else prompt_data.get('enhancement_method', 'huggingface'),
